...
**`sso.png` and `storage.png` are *representative* mockups, not raw captures.** They were built to match the real app's live SSO and File-storage pages (structure taken from real dark-theme screenshots the owner provided), then rendered on a **white background** with the org shown as **Acme Inc** / user **Jordan Avery** and sample values (tenant ID `9b1f8c2e-…`, host `acme.sharepoint.com`) so they match the existing light-theme screenshot set. If the app's SSO/storage UI changes, regenerate them from the real pages the same way rather than hand-editing the PNGs. The HTML+headless-Chrome generator (`build.js`) lives in the session scratchpad, not the repo.
