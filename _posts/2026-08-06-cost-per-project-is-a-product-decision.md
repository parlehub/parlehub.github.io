...
This is not neglect. In most companies the number does not exist in a form anyone could act on. The provider invoice arrives as one figure. Finance codes it to overhead. The initiative continues, because continuing is free and cancelling requires an argument, and an argument requires evidence.

*When did your organization last cancel an AI project?*

For most, the honest answer is never. Not because every initiative earned its keep, but because nothing ever surfaced that would force the conversation. A year ago we argued that AI spend needs to be [tracked and budgeted at the project level](/blog/2026/07/24/why-enterprise-ai-needs-project-level-tracking/) rather than in aggregate. That argument was about visibility. This one is about what visibility is *for*, and the answer is not a better report.

Consider a restaurant that knows precisely what it spent at the wholesaler last month and has no idea what any individual dish costs to produce. It can file accurate accounts. It cannot design a menu. It cannot tell which plates carry the kitchen and which are quietly subsidised by the ones that do, it cannot price anything deliberately, and it certainly cannot decide what to stop making. The grocery bill is a true number that supports no decision at all.
...
The result is a portfolio that only ever grows. New pilots are added each quarter and none are retired, not because they are all working, but because the organization has no mechanism for concluding that one is not. Every mature company has a portfolio review process for capital projects. Very few have one for AI, and the reason is that a review with no numbers is just a meeting.
...
This is the oldest failure mode in shared infrastructure and it has an entirely predictable outcome. Demand rises to meet whatever the platform team can absorb, the platform team's budget becomes the de facto ceiling on company-wide AI adoption, and the argument each year is about the platform team's line item rather than about whether any specific piece of work deserved the spend. The cost sits in the wrong place to inform the decision that caused it.
...
A return figure without a denominator is not a weak business case, it is not a business case. It cannot be compared against another initiative, ranked in a portfolio, or defended when someone senior asks the obvious question. Teams doing genuinely valuable work end up unable to prove it, which is a worse outcome than the wasteful pilot surviving, because it means the good work cannot be funded harder.
...
Per-user attribution fails this test, and it fails it hardest for the people doing the most valuable work. Rae is on four client accounts this week. Attributing their consumption to their name produces a number that describes nobody's decision and supports no action. Split the same consumption across the four engagements and every figure becomes something a manager can actually do something about.
...
Any firm that delivers work for other companies already runs this discipline on every other input. Hours carry a client code. Disbursements carry a client code. AI is typically the only input in the building that arrives with no code on it, which means it is the only input the business cannot price. The moment it carries the same code as everything else, it stops being a cost centre to be squeezed and becomes an input to be managed, and some of it stops being your cost at all.
...
* **Budgets are scoped to projects, not just to people.** A project maps to whatever you actually make decisions about: an initiative, an engagement, a matter, a client. Personal and organization-wide caps stack above as backstops. This is the answer to *The Pilot That Cannot Fail* and to *Cost Has to Land on Something You Can Cancel*, because an initiative with its own ledger is an initiative someone can choose to stop.
* **Enforcement happens before the spend, not after it.** A request that would exceed the project's budget is blocked pre-flight, with a warning state at 80% so the team sees it coming, and a one-click increase request routed to an admin for approval or rejection. It is a hard stop with a path through it rather than a report of what already happened, which is what turns *AI as Weather* back into a decision somebody makes.
* **Every project carries your own expense code.** Projects hold the code your finance team already uses, and it travels through to the export, so AI cost joins your client master on the same key as every other cost. This is what makes *Cost You Can Move Is Cost You Can Price* real rather than aspirational.
* **One ledger behind both the report and the enforcement.** Spend and token usage break down by project, person, provider, or model over any time range, with CSV export, all drawn from the same canonical ledger the budgets enforce against. There is one number, which is the only way *Return With No Denominator* ever gets fixed.
