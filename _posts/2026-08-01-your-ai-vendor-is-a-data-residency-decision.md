...
* **The record of who did what is not editable.** Sensitive actions are written to an audit log that is append-only, enforced at the core platform level rather than by application code, so even a compromised application credential cannot quietly rewrite history.
